...
export default function HeroSlider() {
	...
	const LeftHandler = () => {
		if (ActiveIndex > 1) {
			// Checking if ActiveIndex is greater than 0 to avoid going out of bounds
			setActiveIndex(ActiveIndex - 1); // Decrementing ActiveIndex to move to the previous slider
		}
	};

	const RightHandler = () => {
		if (ActiveIndex < SlidersArray.length) {
			// Checking if ActiveIndex is within the array bounds
